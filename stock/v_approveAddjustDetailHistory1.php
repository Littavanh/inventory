	...
	<div class="row">
		<div class="col-md-12">
			<div class="box box-primary">
				...
				onclick="document.location='?d=stock/approveAddjustHistory1'">ປິດ</button>
				<br>
					<?php
					...
					while ($rowApprove = mysql_fetch_array($approve, MYSQL_ASSOC)) {
						...
						<div class="row">
							...
							<div class="form-group col-md-3">
								...
								<input type="text" class="form-control" value="<?= $rowApprove['first_name'] ?> <?= $rowApprove['last_name'] ?>" readonly />
							</div>
							<div class="form-group col-md-3">
								<label>level</label>
								<input type="text" class="form-control" value="<?= $rowApprove['approve_level'] ?>" readonly />
							</div>
							<div class="form-group col-md-3">
					...
					<div class="row">
						<div class="box-body pad table-responsive">
							...
							<table id="example3" class="table table-bordered table-hover beautified editable">
								...
								<tbody>
									<?php
									$load = loadAddjustHistory($transferID);
									$i = 1;
									while ($row = mysql_fetch_array($load, MYSQL_ASSOC)) {
										...
										<tr>
											...
											<td class="centered">
												<?= number_format($row['pur_price'] * $row['unitQty3'] * (-1), 2, '.', ',') ?>
											</td>
											<td>
												<?= $row['reason'] ?>
											</td>
