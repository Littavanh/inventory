...
if (isset($_POST['type']) && $_SESSION['EDPOSV1role_id'] <= 3  ) {
	for ($i = 0; $i < count($_POST['type']); $i++) {	
		...
		if ($_POST['txtkf_name'][$i] != ""  ) {	
			switch ($_POST['type'][$i]) {	
				case "added" :	
					if (mysql_num_rows(mysql_query("SELECT * FROM tb_kind_food_ WHERE kf_name = '$kf_name' and status_id = 3 and info_id='$infoID'")) == 0) {						
						
						sql_execute("INSERT INTO tb_kind_food_(kf_name, status_id,user_add,date_add,user_edit,date_edit, info_id, viewOrder) 
				...
				case "edited":							
					sql_execute(" UPDATE tb_kind_food_ SET kf_name = '$kf_name', info_id='$infoID',
...
if(isset($_GET["del_id"]) &&(($_SESSION['EDPOSV1role_id']) <= 3)){
	...
	$sql = "UPDATE tb_kind_food_ SET status_id = 20, user_edit = '$usr_id', date_edit = NOW() WHERE kf_id ='$id' and info_id='$info_idDel'";	 
	$result = mysql_query($sql, $conn);	

	$sql = "UPDATE tb_food_drink_ SET status_id = 20, user_edit = '$usr_id', date_edit = NOW() WHERE kf_id ='$id' and info_id='$info_idDel'";	 
	$result = mysql_query($sql, $conn);	
	
	if($result){
		header("Location: ?d=manage/category1&infoID=$info_idDel");	
	}else{
