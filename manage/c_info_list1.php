...
if (isset($_POST['btnsave']) && isset($_POST['type']) && $_SESSION['EDPOSV1role_id'] <= 3) {
	for ($i = 0; $i < count($_POST['type']); $i++) {
		...
		$printReceive = "1";
		$saveInfoCus = "1";
		$maxInfoID = nr_execute("SELECT MAX(info_id)+1 as maxInfoID FROM tb_info_ ");
		...
		if ($_POST['txtinfo_name'][$i] != "" && $_POST['txtinfo_tel'][$i] !="") {	
			switch ($_POST['type'][$i]) {	
				case "added" :	
					if (mysql_num_rows(mysql_query("SELECT * FROM tb_info_ WHERE info_name = '$info_name' and info_tel='$info_tel' and status_id IN (1) ")) == 0) {	
						sql_execute("insert into tb_info_ (info_id, info_name, info_addr, info_tel, info_owner, bill_no, 
									...
									FROM tb_info_ WHERE info_id='$Get_infoID' ");
						/* insert material */
						$maxInfoID_new = nr_execute("SELECT MAX(info_id) as maxInfoID FROM tb_info_ ");
						sql_execute("insert into tb_material_ (materialID, info_id, kf_id, mBarcode, materialName, materialRemark, materialRemark1, materialRemark2, 
									...
									FROM 	tb_material_ WHERE info_id='$Get_infoID' and materialID = 1");
						/* insert unit */
				...
				case "edited":		
					...
					/*	sql_execute(" UPDATE tb_pay SET paymentID='$paymentID', pay_amount='$txtPayAmount', pay_date='$txtpayDate',  
									 remark='$txtremark', status_id='6', user_add='$user_id', date_add=NOW()
										WHERE payID = '$payID' " );
						*/
						sql_execute("UPDATE tb_info_ SET info_name='$info_name', info_addr='$info_addr', info_tel='$info_tel', info_owner='$info_owner', 
...
if(isset($_POST["btnsave_image"]) && $_SESSION['EDPOSV1role_id'] <=3){
	if(trim($_FILES["edit_fileUpload"]["tmp_name"]) != "")
	{
		...
		sql_execute("UPDATE tb_info_ SET info_logo = '$file_name'  WHERE  info_id = '".$_SESSION['EDPOSV1show_food_id']."' ");
	/*
...
if(isset($_POST["btnDel_image"]) && $_SESSION['EDPOSV1role_id'] <=3 ){	
	...
	sql_execute("UPDATE tb_info_ SET info_logo = ''  WHERE  info_id = '".$_SESSION['EDPOSV1show_info_id']."' ");
...
if(isset($_GET["del_id"]) && $_SESSION['EDPOSV1role_id'] <= 3 ){
	...
	$sql = "UPDATE tb_info_ SET status_id = 20  WHERE info_id ='$id'";
	$result = mysql_query($sql, $conn);	
	if($result){
		header("Location: ?d=manage/info_list1");
	}else{
