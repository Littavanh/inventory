...
function SumMaterial_CountRow($whereclause) {
	return mysql_query("SELECT materialID, mBarcode, materialName,SUM(unitQty3) as unitQty3, 
						 cap1,  cap2, cap3, unitName1, unitName2,unitName3, SUM(pur_price*unitQty3) as pur_price , exp_date
						 from v_stock_movement $whereclause  GROUP BY materialID, exp_date, pur_price  ");
}
$catcount=0;
...
//$catcount = nr_execute("SELECT COUNT(*) FROM v_stock_movement $whereclause  GROUP BY materialID, exp_date, pur_price ");
...
function SumMaterial($whereclause, $limitclause) {
	...
	return mysql_query("SELECT materialID, mBarcode, materialName, SUM(unitQty3) as unitQty3, 
						 cap1,  cap2, cap3, unitName1, unitName2,unitName3, SUM(pur_price*unitQty3) as pur_price , exp_date
						 from v_stock_movement $whereclause  GROUP BY materialID, exp_date, pur_price  $limitclause ");
}
