...
*/
// --------------------------------------------------------- Build Paging
$catcount = nr_execute("SELECT COUNT(SUM(unitQty3)) FROM v_stock_movement_ $whereclause GROUP BY info_id, materialID, exp_date, pur_price HAVING SUM(unitQty3) >0 ");

function SumMaterial_CountRow($whereclause) {
	return mysql_query("SELECT sum(unitQty3) FROM v_stock_movement_ $whereclause GROUP BY info_id, materialID, exp_date, pur_price HAVING SUM(unitQty3) >0 ");
}
$catcount=0;
...
function SumMaterial($whereclause, $limitclause) {
	return mysql_query("SELECT info_id, mBarcode, materialID, materialName, SUM(unitQty3) as unitQty3, 
						cap1,  cap2, cap3, unitName1, unitName2,unitName3, exp_date, pur_price,cur_name
						from v_stock_movement_ $whereclause GROUP BY info_id, materialID, exp_date, pur_price HAVING SUM(unitQty3) >0 $limitclause");
}
