...
if (isset($_POST['btnsave']) && isset($_POST['type']) && $_SESSION['EDPOSV1role_id'] <= 3) {
	for ($i = 0; $i < count($_POST['type']); $i++) {
		...
		$info_name = encrypt_decrypt('encrypt', mysql_real_escape_string(stripslashes($_POST['txtinfo_name'][$i])));
		$info_addr = encrypt_decrypt('encrypt', mysql_real_escape_string(stripslashes($_POST['txtinfo_addr'][$i]))); 
		$info_tel = encrypt_decrypt('encrypt', mysql_real_escape_string(stripslashes(trim($_POST['txtinfo_tel'][$i])))); 
		$info_owner = encrypt_decrypt('encrypt', mysql_real_escape_string(stripslashes(trim($_POST['txtinfo_owner'][$i]))));
		//$bill_no = mysql_real_escape_string(stripslashes(trim($_POST['txtpayDate'][$i])));
		//$bill_date = mysql_real_escape_string(stripslashes(trim($_POST['txtremark'][$i])));
		$printReceive = mysql_real_escape_string(stripslashes(trim($_POST['txtprintReceive'][$i])));
		$saveInfoCus = mysql_real_escape_string(stripslashes(trim($_POST['txtsaveInfoCus'][$i])));
		$maxInfoID = nr_execute("SELECT MAX(info_id)+1 as maxInfoID FROM tb_info ");
		//$GET_regNo = nr_execute("SELECT MAX(info_id)+1 as maxInfoID FROM tb_info ");
		//$GET_regkey = nr_execute("SELECT MAX(info_id)+1 as maxInfoID FROM tb_info ");
		//$GET_reglicense = nr_execute("SELECT MAX(info_id)+1 as maxInfoID FROM tb_info ");
		/*--------------------------------------------------------------*/
				...
				case "added" :	
					if (mysql_num_rows(mysql_query("SELECT * FROM tb_info WHERE info_name = '$info_name' and info_tel='$info_tel' and status_id IN (1) ")) == 0) {	
						sql_execute("insert into tb_info (info_id, info_name, info_addr, info_tel, info_owner, bill_no, 
									bill_date, printReceive, saveInfoCus, status_id, dbch, regNo, regkey, reglicense, info_logo, exp_num, shopType)
									SELECT '$maxInfoID', '$info_name', '$info_addr', '$info_tel', '$info_owner', 100, NOW(), '$printReceive', '$saveInfoCus', 1,1, regNo, regkey,reglicense, info_logo,1,1
									FROM tb_info WHERE info_id='$Get_infoID' ");
						/* insert material */
						$maxInfoID_new = nr_execute("SELECT MAX(info_id) as maxInfoID FROM tb_info ");
						sql_execute("insert into tb_material (materialID, info_id, kf_id, mBarcode, materialName, materialRemark, materialRemark1, materialRemark2, unitID1, 
									unitQty1, unitID2, unitQty2, unitID3, unitQty3, min_stock, min_unitID, status_id, user_add, date_add, ingredient, mOpenStock, dbch)
									SELECT 	materialID, '$maxInfoID_new', kf_id, mBarcode, materialName, materialRemark, materialRemark1, materialRemark2, unitID1, 
										unitQty1, unitID2, unitQty2, unitID3, unitQty3, min_stock, min_unitID, status_id, '$user_id', NOW(), ingredient, mOpenStock, 1
									FROM 	tb_material WHERE info_id='$Get_infoID' and materialID = 1");
						/* insert unit */
						sql_execute("insert into tb_unit (unitID, info_id, unitName, status_id, user_add, date_add, dbch)
									SELECT 	unitID, '$maxInfoID_new', unitName, status_id, '$user_id', NOW(), 1
									FROM 	tb_unit WHERE info_id='$Get_infoID' and  unitID = 1");
