...
if (isset($_POST['btnSaveProduct']) && isset($_SESSION['EDPOSV1role_id']) && $_SESSION['EDPOSV1role_id'] != 4) {
	...
	if ($exist == "") {
		...
		if ($txtBarcode3 != '') {
			...
			// } else { $exist =''; }
			$query = "SELECT * FROM tb_food_drink WHERE status_id IN (8) and (fd_Barcode = '$txtBarcode3' or fd_name = '$txtm_name') and info_id='$infoID'";
		...
		if ($txtBarcode2 != '') {
			if (mysql_num_rows(mysql_query("SELECT * FROM tb_material WHERE status_id IN (3) and (mBarcode = '$txtBarcode2' or materialName = '$newItemName2' ) and info_id='$infoID' ")) != 0) {
				$exist = "2.1 ບໍ່ສາມາດເພີ່ມໄດ້ເພາະວ່າ " . $txtBarcode2 . " , ມີໃນຖານຂໍ້ມູນແລ້ວ ...!";
			} else {
				$exist = '';
			}
			if (mysql_num_rows(mysql_query("SELECT * FROM tb_food_drink WHERE status_id IN (8) and (fd_Barcode = '$txtBarcode2' or fd_name = '$newItemName2' ) and  and info_id='$infoID' ")) != 0) {
		...
		if ($txtBarcode1 != '') {
			if (mysql_num_rows(mysql_query("SELECT * FROM tb_material WHERE status_id IN (3) and (mBarcode = '$txtBarcode1' or materialName = '$newItemName3' ) and info_id='$infoID' ")) != 0) {
				$exist = "3.1 ບໍ່ສາມາດເພີ່ມໄດ້ເພາະວ່າ " . $txtBarcode1 . " , ມີໃນຖານຂໍ້ມູນແລ້ວ ...!";
			} else {
				$exist = '';
			}
			if (mysql_num_rows(mysql_query("SELECT * FROM tb_food_drink WHERE status_id IN (8) and (fd_Barcode = '$txtBarcode1' or fd_name = '$newItemName3' ) and  and info_id='$infoID' ")) != 0) {
		...
		if (mysql_num_rows(mysql_query("SELECT * FROM tb_material WHERE mBarcode = '$txtBarcode3' and status_id IN (3) and info_id='$infoID' ")) != 0) {
