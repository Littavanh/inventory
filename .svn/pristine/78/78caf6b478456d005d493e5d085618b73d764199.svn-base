...
function LoadPrice_setting($whereclause) {
	return mysql_query("select * from v_material_  $whereclause ");
		
	
}
 
 function selkindfood($SetInfoID){
 	return mysql_query("select * from tb_kind_food_ WHERE status_id=3 and info_id = '$SetInfoID' order by kf_id ");	 
}
...
function LoadInfoByID($info_ID) { 
    return mysql_query("select distinct info_id, info_name, info_id from tb_info_ WHERE status_id IN (1) and info_ID='$info_ID'  ");
