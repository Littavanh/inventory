...
{
    return mysql_query("select * from tb_approval_setting where approve_level ='$approve_level' and status_approve_id = '$status_approve_id' and status_approve_name ='ຮັບສິນຄ້າ(ຄັງຍ່ອຍ)'");
}
...
if (isset ($_GET["approve"])) {
    ...
    while ($row = mysql_fetch_array($load, MYSQL_ASSOC)) {
        ...
        $checkMax = mysql_query("select max(approve_level) from tb_approval_setting where status_approve_id = '$status_approve_id' and status_approve_name ='ຮັບສິນຄ້າ(ຄັງຍ່ອຍ)'");
