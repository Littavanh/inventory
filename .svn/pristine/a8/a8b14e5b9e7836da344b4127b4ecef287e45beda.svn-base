...
function Loadbill($od_no, $Reprint, $infoID) {		
	if ($Reprint == "1") {		
		return mysql_query(" SELECT CONCAT(fd_name,'|',remark) as fd_name1, fd_name,remark, SUM(qty) AS qty, price, (SUM(qty)*price) AS total, tb_name  
						FROM `v_service_detail` WHERE od_no = '$od_no' AND printNo=2 AND printTime=0 and info_id='$infoID' GROUP BY fd_name, price ");
	} else  {
		return mysql_query(" SELECT CONCAT(fd_name,'|',remark) as fd_name1, fd_name,remark, SUM(qty) AS qty, price, (SUM(qty)*price) AS total, tb_name  
						FROM `v_service_detail` WHERE od_no = '$od_no' AND printNo=2 and info_id='$infoID' GROUP BY fd_name, price ");
	}
