...
if (isset($_POST["btnSave"]) && isset($_SESSION['EDPOSV1role_id'])) {
	if (isset($_POST['type'])) {
		...
		$catcount = nr_execute("SELECT COUNT(*) FROM tb_transaction_ WHERE traID = '$trantmp' and info_id='$infoID' ");
		if ($catcount > 0) {
			sql_execute("DELETE FROM tb_import_ WHERE traID='$trantmp' and info_id='$infoID' ");
			sql_execute("DELETE FROM tb_import_detail_ WHERE tranID='$trantmp' and info_id='$infoID' ");
		}
		//============= INSERT HEADER
		if ($_SESSION['EDPOSV1tmpProductID'] != '' && $_SESSION['EDPOSV1AddProducttxtDate'] != '' && $_SESSION['EDPOSV1AddProducttxtreciever'] != '') {
			...
			$sql = "INSERT INTO tb_import_(tranID, traDate, reciever, remark, status_id, user_add, date_add, active_id, openID, supplierID, info_id,bill_no,bill_date,whouse_no,whouse_date,po_no,po_date,lot_no,po_file,statusApprove_id,approver_id,orderer) 
					VALUES('$trantmp', '$txtDate', '$txtreciever', '$txtRemark', '1','$user_id',NOW(),1,'$OpenStockID', '$txtSupplier', '$infoID','$txtBillNo','$txtBill_date','$txtWhouse_no','$txtDate_whouse_date','$txtPo_no','$txtPo_date','$txtLot','$file_name1','2','2613','$txtOrderer') ";
			$resultH = mysql_query($sql, $conn);
		}

		//============ INSERT Detail
		if ($resultH) {
			sql_execute("INSERT INTO tb_import_detail_(tranID, date_tran, exp_date, materialID, unitQty3, tranType, user_add, date_add,status_id, 
					...
					from v_tmp_transactiond_   WHERE tranID='$trantmp' and info_id='$infoID' ");

			$sumAmountTran = nr_execute("SELECT SUM(pur_price*unitQty3) - SUM(receive_dis*unitQty3) FROM v_transaction_ WHERE tranID = '$trantmp' and info_id='$infoID' ");
			...
			// delete from tmp
			sql_execute("DELETE FROM tmp_transaction_ WHERE traID='$trantmp' and info_id='$infoID'");
			sql_execute("DELETE FROM tmp_transactiond_ WHERE tranID='$trantmp' and info_id='$infoID' ");
			//=========== Unset SESSION
...
if (isset($_POST["btnAddproduct"]) && isset($_SESSION['EDPOSV1role_id'])) {
	...
	$trantmp = $_SESSION['EDPOSV1tmpProductID'];
	sql_execute("DELETE FROM tmp_transaction_ WHERE DATE_FORMAT(date_add, '%Y%m%d')  < DATE_FORMAT(now(), '%Y%m%d') and info_id = '$infoID' ");
	sql_execute("DELETE FROM tmp_transactiond_ WHERE DATE_FORMAT(date_add, '%Y%m%d') < DATE_FORMAT(now(), '%Y%m%d') and info_id = '$infoID' ");
	// header("Location: ?d=stock/recieve");
}

if (isset($_POST["btncancel"]) && isset($_SESSION['EDPOSV1role_id'])) {
	unset($_SESSION['EDPOSV1AddProduct']);
	header("Location: ?d=stock1/recieve");
}
			...
			if (($unitQty1 > 0) || ($unitQty2 > 0) || ($unitQty3 > 0)) {
				switch ($_POST['type'][$i]) {
					...
					case "edited":
						...
						sql_execute("INSERT INTO tmp_transactiond_(tranID, exp_date,materialID, traTypeID, unitQty1, unitQty2, unitQty3, 
									...
									FROM v_material2_ WHERE materialID = '$materialID'  and info_id='$infoID'");
						break;
				}
			}
			//======================================= Unset SESSION
...
if (isset($_GET["del_id"]) && isset($_SESSION['EDPOSV1role_id'])) {
	...
	$sql = "DELETE FROM tmp_transactiond_ WHERE traDID ='$id' and info_id = '$delinfoid' ";
	echo "DELETE FROM tmp_transactiond_ WHERE tmpDID ='$id' and info_id = '$delinfoid' ";
...
$infoID = $_SESSION['EDPOSV1AddProducinfoID'];
// --------------------------------------------------------- Build Paging
$catcount = nr_execute("SELECT COUNT(*) FROM v_tmp_transactiond_ where info_id = '$infoID' ");
