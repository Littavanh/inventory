...
if (isset($_POST['type']) && $_SESSION['EDPOSV1role_id'] <= 3  ) {
	for ($i = 0; $i < count($_POST['type']); $i++) {	
		...
		if ($txtSuppliername != "" &&  $ContactName!='' && $txtTel !='') {	
			switch ($_POST['type'][$i]) {	
				case "added" :	
					if (mysql_num_rows(mysql_query("SELECT * FROM tb_supplier WHERE (tel = '$txtTel' or supplierName='$txtSuppliername') and status_id =1 ")) == 0) {
						sql_execute("INSERT INTO tb_supplier(supplierName, ContactName, tel, addr, email, remark, status_id, user_add, date_add) 
				...
				case "edited":		
					//if (mysql_num_rows(mysql_query("SELECT * FROM tb_supplier WHERE (tel = '$txtTel' or supplierName='$txtSuppliername') and supplierID <>'$supplierID' ")) == 0) {
						sql_execute(" UPDATE tb_supplier SET supplierName = '$txtSuppliername', ContactName='$ContactName', tel='$txtTel', email='$txtemail',
...
if(isset($_GET["del_id"]) &&(($_SESSION['EDPOSV1role_id']) <= 3)){
	...
	$sql = "UPDATE tb_supplier SET status_id = 20  WHERE supplierID ='$id'";
	echo $sql;
	$result = mysql_query($sql, $conn);	
	if($result){
		header("Location: ?d=manage/supplier");
	}else{
		echo "<center><h2>ERROR Delete</h2></center>";
	}
}

$catcount = nr_execute("SELECT COUNT(*) FROM tb_supplier $whereclause");
