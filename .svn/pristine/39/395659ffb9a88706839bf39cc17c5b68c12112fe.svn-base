...
function SumMaterial($whereclause) {
	...
	return mysql_query("SELECT  materialID
							from v_transaction $whereclause and  active_id IN (1,2) and tranType IN (1,15,20)  GROUP BY materialID ORDER BY date_add ");
...
function LoadLot(){
	return mysql_query(" SELECT DISTINCT lot_no FROM v_transaction");
}
function LoadTableAll(){
	return mysql_query("select * from v_transaction GROUP BY materialID ORDER BY materialID, date_add");
}
// count currency 
