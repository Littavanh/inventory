								...
								<tbody>
									...
									while ($row = mysql_fetch_array($load, MYSQL_ASSOC)) {
										...
										<tr>
											...
											<td class="centered">
												<a
													href="index.php?d=stock1/approveRecieveDetailHistory&tranID=<?= $row['tranID'] ?>&po_no=<?= $row['po_no'] ?>&po_date=<?= $row['po_date'] ?>&info_name=<?= $row['info_name'] ?>&bill_no=<?= $row['bill_no'] ?>&bill_date=<?= $row['bill_date'] ?>&whouse_no=<?= $row['whouse_no'] ?>&whouse_date=<?= $row['whouse_date'] ?>"><i>ເບິ່ງລາຍລະອຽດ</i></a>
											</td>
											<td class="centered"><a class="btn btn-warning"
													href="print/approveRecievePrint_.php?d=stock1&tranID=<?= $row['tranID'] ?>"
