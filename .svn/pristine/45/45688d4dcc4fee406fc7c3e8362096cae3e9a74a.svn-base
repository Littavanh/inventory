...
if(isset($_GET["del_id"]) && isset($_SESSION['EDPOSV1role_id']) && $_SESSION['EDPOSV1role_id'] != 4){	
	...
	$sql = "DELETE FROM tb_material   WHERE materialID ='$id' and info_id='$GetInfoID' ";
	$result = mysql_query($sql, $conn);	
	// delete transection
	$sql = "DELETE FROM tb_transactiond   WHERE materialID ='$id' and info_id='$GetInfoID'";
	$result = mysql_query($sql, $conn);	
	// upate food list
		$rsRecipe = mysql_query("SELECT * FROM tb_recipe WHERE materialID ='$id' and info_id='$GetInfoID' ");	
		while($row = mysql_fetch_array($rsRecipe, MYSQL_ASSOC)) {
			$recipeID = $row['recipeID'];
			$fdID = $row['fd_id'];

			// recipe ID 
			//$sql = "UPDATE tb_recipe SET status_id = 20, user_edit = '$usr_id', date_edit = NOW() WHERE recipeID ='$recipeID' and info_id='$GetInfoID' ";
			$sql = "DELETE FROM tb_recipe   WHERE recipeID ='$recipeID' and info_id='$GetInfoID' ";
			$result = mysql_query($sql, $conn);	
			// fd list
			//$sql = "UPDATE tb_food_drink SET status_id = 20, user_edit = '$usr_id', date_edit = NOW() WHERE fd_id ='$fdID' and info_id='$GetInfoID'";
			$sql = "DELETE FROM tb_food_drink   WHERE fd_id ='$fdID' and info_id='$GetInfoID'";
			$result = mysql_query($sql, $conn);	
