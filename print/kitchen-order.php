...
<html>
  ...
  <?php 
      ...
      $countRecord=0;
      if ($Reprint == "1") {
        $countRecord = nr_execute("SELECT COUNT(*)  FROM v_service_detail WHERE od_no = '$bill_id' AND printNo=2 AND printTime=0 and info_id ='$infoID' ");
      } else if ($Reprint == "2") {
        $countRecord = nr_execute("SELECT COUNT(*)  FROM v_service_detail WHERE od_no = '$bill_id' AND printNo=2  and info_id ='$infoID' ");
      }
    ...
    <table>
	  ...
	  <div id="to_control">
        ...
        <tr>
            ...
            <? $i++; } 
              if ($Reprint == "1") {        
                  sql_execute("UPDATE tb_order_detail SET printTime=2 WHERE od_no = '$od_no' AND printTime<=1 AND printNo=2 and info_id ='$infoID' ");
                } else if ($Reprint == "2") {
                  sql_execute("UPDATE tb_order_detail SET printTime=printTime+1 WHERE od_no = '$od_no' AND printNo=2 and info_id ='$infoID' ");                 
                }
            ?>            
		</table>	       
	  </div>
